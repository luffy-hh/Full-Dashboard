const Thai2DNumEvening = require("../models/thai2DNumEveningModel");
...
exports.read2dAllNum = async (req, res) => {
  try {
    ...
    const query = Thai2DNumEvening.find(JSON.parse(queryStr));
...
exports.update2DNum = async (req, res) => {
  try {
    ...
    const updateSingle2DNumField = await Thai2DNumEvening.findByIdAndUpdate(
      id,
      { ...req.body },
      { new: true }
    );
